...
module.exports = {
  ...
  async execute(client, message,args,prefix) {
    ...
    // Check if the guild has the autoModerationRules property available
    if (!message.guild.autoModerationRules) {
      return message.channel.send({
        embeds: [
          new EmbedBuilder()
            .setColor(process.env.color)
            .setDescription(`${client.emoji.cross} | **AutoMod is not available for this server. Make sure the server has the proper boost level.**`)
        ]
      });
    }
    if (!message.guild.members.me?.permissions.has(PermissionsBitField.Flags.Administrator)) {
        return message.channel.send({ 
            embeds: [new EmbedBuilder()
                .setColor(process.env.color)
                .setDescription(`${client.emoji.cross} | **I don't have \`Administrator\` Permissions here**`)]
        });
    }
    ...
    let msg = await message.channel.send({
        embeds: [new EmbedBuilder()
            .setColor(process.env.color)
            .setDescription(`**<a:stolen_emoji_or_sticker:1426093099326771241> | Creating Automod Rules**`)]
    });

    const rule = await message.guild.autoModerationRules.create({
        name: `Harmonia AutoMod Rule 1`,
    ...
    const rule2 = await message.guild.autoModerationRules.create({
        ...
        const errorEmbed = new EmbedBuilder()
            .setColor(process.env.color)
            .setDescription(`${client.emoji.cross} **Automod \`Rule 2\` is already Enabled In This Server.**`);
        return message.reply({ embeds: [errorEmbed] });
    });

    const rule3 = await message.guild.autoModerationRules.create({
        name: `Harmonia AutoMod Rule 3`,
        creatorId: client.user.id,
        enabled: true,
        eventType: 1,
        triggerType: 3,
        ...
        const errorEmbed = new EmbedBuilder()
            .setColor(process.env.color)
            .setDescription(`${client.emoji.cross} **Automod \`Rule 3\` is already Enabled In This Server.**`);
        return message.reply({ embeds: [errorEmbed] });
    });

    const rule4 = await message.guild.autoModerationRules.create({
        name: `Harmonia AutoMod Rule 4`,
        creatorId: client.user.id,
        enabled: true,
        eventType: 1,
        triggerType: 5,
        ...
        const errorEmbed = new EmbedBuilder()
            .setColor(process.env.color)
            .setDescription(`${client.emoji.cross} **Automod \`Rule 4\` is already Enabled In This Server.**`);
        return message.reply({ embeds: [errorEmbed] });
    });
    ...
    setTimeout(async () => {
        ...
        const embed = new EmbedBuilder()
            .setAuthor({
                ...
                iconURL: `https://cdn.discordapp.com/attachments/1223515094651240531/1228309835855036447/632a24dc0918d34459738418_AutoMod-Final-Blog-Author.png`
            })
            .setColor(process.env.color)
            .setDescription(`**${client.emoji.tick} | All 4 Automod Rules Have Been Created Successfully.**`)
            .setTimestamp();
