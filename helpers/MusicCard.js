...
class MusicCard {
	...
	async generateNowPlayingCard(options) {
		const { track, position = 0, isLiked = false, guildName = 'Discord Server', guildIcon = null, player, cardStyle = 'large' } = options;
		
		if (cardStyle === 'small') {
			return this.generateSmallNowPlayingCard(options);
		}
		...
		const isLive = !track?.info?.length || track.info.length <= 0;
		const currentTime = this.formatDuration(position);
		const totalTime = isLive
			? 'LIVE'
			: this.formatDuration(track?.info?.length || 0);
		...
		ctx.fillText('Harmonia Music', width - margin - 10, bottomY);
	...
	async generateSmallNowPlayingCard(options) {
		const { track, position = 0, isLiked = false, guildName = 'Discord Server', guildIcon = null, player } = options;
		
		// Small card dimensions
		const width = 400;
		const height = 120;
		const margin = 15;
		const artworkSize = 90;
		...
		const artworkX = margin;
		const artworkY = margin;
		const infoX = artworkX + artworkSize + 15;
		const contentWidth = width - infoX - margin;
		...
		ctx.shadowOffsetY = 0;
		ctx.fillStyle = '#a0b0c0';
		ctx.fillText(`${source}`, infoX, currentY);
		ctx.restore();
		...
		ctx.fillStyle = 'rgba(0, 0, 0, 0.5)';
		ctx.fillText(displayTitle, infoX + 1, currentY + 1);
		...
		ctx.shadowBlur = 6;
		ctx.fillStyle = '#e0e8f0';
		ctx.font = '12px "Inter SemiBold"';
		ctx.fillText('Harmonia Music', width - margin - 5, bottomY);
